...
pub async fn create_update_user_note(
    ...
    let res = models::schedule::is_time_link_id_valid_for_user(
        &mut db,
    ...
    //insert entry
    models::notes::create_update_user_note(
        &mut db,
        auth.user_id,
        data.schedule_obj_time_link_id,
        data.text.clone(),
...
pub async fn get_user_notes(mut db: Connection<Db>, auth: Option<AuthorizeInfo>) -> GetRes {
    ...
    let map = res
        .iter()
        .map(|(&week_id, notes)| {
            let notes_map: BTreeMap<_, _> =
                notes.iter().map(|note| (note.1, note.0.clone())).collect();

            (
                week_id,
                UserNotesWeek {
                    week_id,
                    user_notes: notes_map,
                },
            )
        })
        .collect();
...
pub async fn create_update_group_note(
    ...
    //insert entry
    models::notes::create_update_group_note(
        &mut db,
        group_id,
        data.schedule_obj_time_link_id,
        data.text.clone(),
...
pub fn get_routes() -> Vec<Route> {
    routes![
        create_update_user_note,
        get_user_notes,
        delete_user_note,
        create_update_group_note,
        get_group_notes,
        delete_group_note
    ]
}
