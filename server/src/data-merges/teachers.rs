...
async fn single_teacher_merge(teacher_id: i32, teacher: &TeacherModel, last_gen_id: i32, con: &mut PgConnection) -> anyhow::Result<MergeResult> {
    ...
    con.transaction(|trx| Box::pin(async move {
        let row : Option<TeacherModel> = models::teachers::get_cur_gen_teacher_by_id(teacher_id, &mut *trx).await?;
        ...
        if let Some(row) = row {
            ...
            if diff {
                ...
                new_gen_id, teacher_id)
                    .execute(&mut **trx).await.context("Failed to invalidate old teacher generation")?;
        ...
        gen_start, existence_diff)
            ...
            $11, $12, $13, $14, $15, $16, $17)",
                ...
                new_gen_id, "changed")
                    .execute(&mut **trx).await.context("Failed to insert teacher in teacher merge")?;
